...
local function genIsComplete(obj)
    if not obj or not obj.Parent then return false end
    local done = obj:GetAttribute("Completed") or obj:GetAttribute("Done") or obj:GetAttribute("Complete")
    if done and done ~= false and done ~= 0 then return true end
    local progress = obj:GetAttribute("Progress")
    if progress and tonumber(progress) and tonumber(progress) >= 100 then return true end
    return false
end

local function genGetProgress(obj)
    if not obj or not obj.Parent then return 0 end
    -- Check completion first (fast path)
    if genIsComplete(obj) then return 100 end
    -- Attribute (fastest)
    local progress = obj:GetAttribute("Progress")
    if progress and tonumber(progress) then return tonumber(progress) end
    -- Direct children only (no GetDescendants to avoid lag)
    for _, child in ipairs(obj:GetChildren()) do
        if (child:IsA("IntValue") or child:IsA("NumberValue")) and child.Name == "Progress" then
            return child.Value
        end
    end
    return 0
...
local function genUpdateLabel(obj)
    pcall(function()
        ...
        if not label then
            ...
            if bb then label = bb:FindFirstChild("TextLabel"); if label then genLabels[obj] = label end end
            if not label then return end
        end
        local progress = genGetProgress(obj)
        local newText
        if progress >= 100 or genIsComplete(obj) then
            newText = "Generator  ✓ DONE"
            label.TextColor3 = Color3.fromRGB(100, 255, 100)
        else
            local puzzles = math.floor(progress / 25)
            newText = string.format("Generator  %d/4  (%d%%)", puzzles, math.floor(progress))
...
local function genWatchGenerator(obj)
    pcall(function()
        ...
        if obj._genConns then
            for _, conn in ipairs(obj._genConns) do pcall(function() conn:Disconnect() end) end
            obj._genConns = nil
        end
        local conns = {}
        -- Watch all completion/progress attributes
        table.insert(conns, obj.AttributeChanged:Connect(function(attr)
            if attr == "Progress" or attr == "Completed" or attr == "Done" or attr == "Complete" then
                genUpdateLabel(obj)
            end
        end))
        -- Watch direct children only (not descendants — too expensive)
        ...
        for _, child in ipairs(obj:GetChildren()) do hookChild(child) end
